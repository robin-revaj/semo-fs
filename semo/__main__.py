...
def main():
    ...
    tag_parser = subparsers.add_parser("tag", help="Assign given tag (optionally with given value) to given filepath. Format: tag [tag_name] [value] [filename]")
    ...
    untag_parser = subparsers.add_parser("utag", help="Remove given tag from given filepath. Format: untag [tag_name] [filename]")
    ...
    list_tags_parser = subparsers.add_parser("ls_tags", help="List all tags and their values associated with the given filename. If no file is provided, list all tags.")
    ...
    list_roots_parser = subparsers.add_parser("ls_roots", help="List root tags. A root tag is a tag that is not a subtag to any other tags.")
    list_roots_parser.set_defaults(func=cli.translate_LISTROOTS)

    list_subtags_parser = subparsers.add_parser("ls_subtags", help="List subtags for given tag.")
    ...
    list_files_parser = subparsers.add_parser("ls_files", help="List files that correspond to query conditions. If no query is provided, lists all files in database.")
    ...
    delete_tag_parser = subparsers.add_parser("deltag", help="Delete tag and all its relationships")
    delete_tag_parser.add_argument("tagname", help="Name of tag")
    delete_tag_parser.set_defaults(func=cli.translate_DELTAG)

    subtag_parser = subparsers.add_parser("subtag", help="Assign subtags to superior tag")
    ...
    usubtag_parser = subparsers.add_parser("usubtag", help="Unassign subtags from superior tag")
    ...
    select_database_parser = subparsers.add_parser("db", help="Select database file to use. If file not provided, return path to current database file.")
    ...
    watch_parser = subparsers.add_parser("watch", help="Add directory to watchlist. Watchlist is used to catch file changes and update tags accordingly.")
    ...
    unwatch_parser = subparsers.add_parser("uwatch", help="Remove directory from watchlist. Watchlist is used to catch file changes and update tags accordingly.")
    ...
    mount_parser = subparsers.add_parser("mount", help="Mount the semo virtual file system at provided mountpoint.")
    ...
    umount_parser = subparsers.add_parser("umount", help="Unmount the semo virtual file system from mountpoint.")
    ...
    clean_parser = subparsers.add_parser("clean", help="Clear the database of damaged or outdated entries.")
    clean_parser.set_defaults(func=cli.translate_CLEAN)

    export_parser = subparsers.add_parser("export", help="Export tag data for given directory to the files' extended attributes.")
    ...
    import_parser = subparsers.add_parser("import", help="Attempt to import tag data in given directory from the available options. Option sets [-a], [-x], [-fi], [-mfi], [-mf] guarantee full recovery. For other combinations (eg. [-m]) only partial recovery may be possible.")
