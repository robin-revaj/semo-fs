...
class Database:
    ...
    def new_file(self, file_system, inode : int, name : str):
        self.__cursor.execute("INSERT INTO file VALUES (NULL, ?, ?, ?)", (file_system, inode, name))
        self.__connection.commit()
    ...
    def list_files(self) -> list[tuple[int, int, str]]:
        res = self.__cursor.execute ("SELECT file_system, inode, name FROM file")
    ...
    def list_tags_for_file(self, file_system, inode : int) -> list[str]:
        ...
        return [x[0] for x in res.fetchall()]
    def list_files_for_tag(self, tag_name : str) -> list[tuple[int, int, str]]:
        res = self.__cursor.execute("SELECT file.file_system, file.inode, file.name FROM (\
                                        SELECT rel_file_tag.tag_id, rel_file_tag.file_id \
    ...
    # filenames
    def get_file_name(self, file_system, inode : int) -> str:
        res = self.__cursor.execute("SELECT name FROM file WHERE file_system == ? AND inode == ?", (file_system, inode))
        return res.fetchone()[0]
    def set_file_name(self, file_system, inode : int, name : str):
        self.__cursor.execute("UPDATE file SET name = ? WHERE file_system == ? AND inode == ?", (name, file_system, inode))
        self.__connection.commit()